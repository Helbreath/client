...
void CGame::UpdateScreen_OnLogResMsg()
{
    ...
    if ((msX >= 451) && (msX <= 451 + BTNSZX) && (msY >= 334) && (msY <= 334 + BTNSZY))
        ...
        DrawNewDialogBox(SPRID_INTERFACE_ND_BUTTON, 451, 334, 0);
...
void CGame::UpdateScreen_OnGame()
{
        ...
        (m_pItemList[m_stMCursor.sSelectedObjectID] != 0))
    {
        ...
        if (ItemColor != 0)
        {
            ...
            m_pSprite[SPRID_ITEMPACK_PIVOTPOINT + m_pItemList[m_stMCursor.sSelectedObjectID]->m_sSprite]->PutSpriteFast(G_pGame->m_stMCursor.sX - m_stMCursor.sDistX, G_pGame->m_stMCursor.sY - m_stMCursor.sDistY,
...
#ifdef USING_WIN_IME
    if (bIsHide == false)
        G_hEditWnd = CreateWindow(RICHEDIT_CLASS, 0, WS_POPUP | ES_SELFIME, sX - 5, sY - 1, len * 12, 16, G_hWnd, (HMENU)0, G_hInstance, 0);
    else
        G_hEditWnd = CreateWindow(RICHEDIT_CLASS, 0, WS_POPUP | ES_PASSWORD | ES_SELFIME, sX - 5, sY - 1, len * 12, 16, G_hWnd, (HMENU)0, G_hInstance, 0);
...
#ifdef USING_WIN_IME
    if (G_hEditWnd != 0)
        GetWindowText(G_hEditWnd, pString, (int)m_inputMaxLen);
#endif
}
