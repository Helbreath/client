...
#include "Fps.h"
#include "ItemBag.h"
// todo - replace all this terrible push/pop code with stream_write/read
#include "buffer.h"
...
#define GAMEFUNCT(x) std::bind(&helbreath::x, this)
...
public:
    ...
    void PutChatString(int iX, int iY, char * pString, Color color = Color(255, 255, 255));
	...
	void PutAlignedString(int iX1, int iX2, int iY, std::string text, int r, int g, int b)
	{
		PutAlignedString(iX1, iX2, iY, text, Color(r, g, b));
	}
   ...
   void DlgBoxClick_GuildContribute();//58
	...
	std::vector<std::string> * GetItemName(char * cItemName, uint32_t attr, uint8_t sockets[MAXITEMSOCKETS], uint32_t count);
	std::vector<std::string> * GetItemName(CItem * pItem, bool isWH = false);
	...
	void UpdateScreen_OnSelectCharacter(short sX, short sY, bool bIgnoreFocus = false);
	...
	bool _bDraw_OnCreateNewCharacter(std::string pName, short msX, short msY, int iPoint);
	...
	uint32_t GetItemCount(const char * pItemName);
	...
	void LogResponseHandler(char * pData, uint32_t dwMsgSize);
	void OnTimer();
	...
	class CMisc      m_Misc;
	class sprite  * m_pSprite[MAXSPRITES];
	class sprite  * m_pTileSpr[MAXTILES];
	class sprite  * m_pEffectSpr[MAXEFFECTSPR];
	...
	CMsg    * m_pWhisperMsg[MAXWHISPERMSG];
	CEffect * m_pEffectList[MAXEFFECTS];
	CItem   * m_pItemList[MAXITEMS];
	CItem   * m_pBankList[MAXBANKITEMS];
	ItemMap m_guildBankMap;
	uint32_t m_guildBankVer;
	CMagic * m_pMagicCfgList[MAXMAGICTYPE];
	CSkill * m_pSkillCfgList[MAXSKILLTYPE];
	CMsg * m_pMsgTextList[TEXTDLGMAXLINES];
	CMsg * m_pMsgTextList2[TEXTDLGMAXLINES];
	CMsg * m_pAgreeMsgTextList[TEXTDLGMAXLINES];
	CBuildItem * m_pBuildItemList[MAXBUILDITEMS];
	CBuildItem * m_pDispBuildItemList[MAXBUILDITEMS];


	CItem * m_pItemForSaleList[MAXMENUITEMS];
    vector<shared_ptr<CCharInfo>> m_pCharList;
	CItemName * m_pItemNameList[MAXITEMNAMES];
	CCurse m_curse;
...
//	int m_iAccountStatus;
	...
	// todo - increase to 64bit for better handling of more mobs spawning between restarts
	...
	CBuildItem * m_pCraftItemList[MAXBUILDITEMS];
	CBuildItem * m_pDispCraftItemList[MAXBUILDITEMS];
    ...
    int iHungerStatus = 100;
    bool m_bStuckHack = false;
...
#endif
	bool bCheckItemEquiped(const char itemName[]); // Beholder Necklace Fix xRisenx
