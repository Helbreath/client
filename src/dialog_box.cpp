...
void dialog_box::SetupDialog(char dialogID, short X, short Y, short sizeX, short sizeY, short background, int backFrame, int titleTxtFrame, bool trans, helbreath * game)
{
    if (game == nullptr)
        throw std::exception("Dialog improperly set up");
...
void dialog_box::SetupHandlers(HandlerFunct draw, HandlerFunct click, HandlerFunct dblClick, HandlerFunct dropItem)
{
    _drawHandler = draw;
    _clickHandler = click;
...
void dialog_box::GetAllButtons(std::vector<RECT> * rects) const
{
...
bool dialog_box::HasBackground(int & sprite, int & frame) const
{
...
bool dialog_box::HasTitleText(int & frame) const
{
...
bool dialog_box::GetTrans() const
{
    return _trans;
}

char dialog_box::GetMode() const
{
    return _mode;
}

void dialog_box::AddButton(short X, short X2, short Y, short Y2, char ID)
{
...
void dialog_box::AddButtonList(short X, short X2, short Y, short Y2, char count, char startingID, short spacing)
{
    int totalGap;
...
void dialog_box::AddScrollBar(short X, short X2, short Y, short Y2, char ID/*, bool vertical*/)
{
    AddButton(X, X2, Y, Y2, ID);
    _scrollBar = ID;
}

char dialog_box::OnButton() const
{
    short X = game_->m_stMCursor.sX - x;
    short Y = game_->m_stMCursor.sY - y;
...
bool dialog_box::IsOnScrollBar()  const
{
...
Point dialog_box::HandleScroll(int visibleAmt, int listSize)
{
    Point ret;
    ...
    if (_scrollBar)
    {
        ...
        ret.x = x + scroll->_X + 10;
        ret.y = y + scroll->_Y + 10;
    }
    else
    {
        ret.x = x;
    ...
    if (listSize >= visibleAmt && _scrollBar) //Adjust scroll only if list large enough
    {
        ...
        {
            if (game_->m_stMCursor.LB != 0)
            {
                if (OnButton() == _scrollBar)// Possibly change to use precalculated onButton value
                {
                    double d1 = game_->m_stMCursor.sY - y - scroll->_Y - 10;
            ...
            if (game_->m_stMCursor.sZ != 0)
            {
                if (d2 > 8) //Scroll slower on smaller lists
                    sView -= game_->m_stMCursor.sZ / 60;
                else
                    sView -= game_->m_stMCursor.sZ / 120;
            }
...
void dialog_box::ClearButtons()
{
    _buttonCount = 0;
    _scrollBar = 0;
}

bool dialog_box::OnDialogBox() const
{
    if (((game_->m_stMCursor.sX) < x)
        || (game_->m_stMCursor.sX > x + width)
        || ((game_->m_stMCursor.sY) < y)
        || (game_->m_stMCursor.sY > y + height))
        return false;
    return true;
}

const RECT dialog_box::GetButton(int buttonID) const
{
    RECT ret = {};
    buttonID--;
    ret.bottom = y + _buttons[buttonID]._Y2;
    ret.top = y + _buttons[buttonID]._Y;
    ret.left = x + _buttons[buttonID]._X;
    ret.right = x + _buttons[buttonID]._X2;
    return ret;
}

const int dialog_box::ButtonX(int buttonID) const
{
    return x + _buttons[buttonID - 1]._X;
}

const int dialog_box::ButtonY(int buttonID) const
{
    return y + _buttons[buttonID - 1]._Y;
}

void dialog_box::ValidateButtons() const
{
#ifdef _DEBUG
    int j, i, err = 0;
    ...
    for (i = 0; i < _buttonCount; i++)
    {
        ...
        if (_buttons[i]._X2 > width) err = 4; //Btn going off right
        if (_buttons[i]._Y2 > height) err = 5; //Btn going bottom
...
void dialog_box::MoveOnScreen(short X, short Y)
{
    if (X != -1) x = X;
    if (Y != -1) y = Y;
    if (x < 0) x = 0;
    //if (m_X + sSizeX > 639) m_X = 639 - sSizeX;
    if (x + width > 799) x = 799 - width; // 800x600 Resolution xRisenx
    if (y < 0) y = 0;
    //if (m_Y + sSizeY > 479) m_Y = 479 - sSizeY;
    if (y + height > 599) y = 599 - height; // 800x600 Resolution xRisenx
}

void dialog_box::CenterOverPoint(short X, short Y)
{
    x = X - width / 2;
    y = Y - height / 2;
    MoveOnScreen();
...
void dialog_box::SetMode(char mode)
{
    ...
    switch (_dialogID) {
        ...
        case 26: //////SkillDlg
            switch (mode) {
                ...
                case 3:
                    ...
                    AddButton(30, width - 31, 345, 360, 16);
        ...
        case 35: //////Help
            AddButtonList(25, width - 26, 50, 50 + 15, 14, 1);
        ...
        case 57: //////Guild
            switch (mode)
            {
                ...
                case 8:
                    ...
                    AddScrollBar(230, 255, 25, 325, 2);
                    AddButton(0, width, 300, 315, 3); // Back
