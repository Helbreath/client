...
void CDialogBox::SetupDialog(char dialogID, short X, short Y, short sizeX, short sizeY, short background, int backFrame, int titleTxtFrame, bool trans)
{
	...
	_dialogID  = dialogID;
    _clickHandler = std::bind(&CDialogBox::voidfunc, this);
    _drawHandler = std::bind(&CDialogBox::voidfunc, this);
    _dblClickHandler = std::bind(&CDialogBox::voidfunc, this);
    _dropItemHandler = std::bind(&CDialogBox::voidfunc, this);
...
{
	short X = game->m_stMCursor.sX - m_X;
	short Y = game->m_stMCursor.sY - m_Y;
...
Point CDialogBox::HandleScroll(int visibleAmt, int listSize)
{
	...
	if (listSize >= visibleAmt && _scrollBar) //Adjust scroll only if list large enough
	{
		...
		if (IsTop()) //Scroll only if top dialog
		{
			...
			if (game->m_stMCursor.sZ != 0)
			{
				if (d2 > 8) //Scroll slower on smaller lists
					sView -= game->m_stMCursor.sZ/60;
				else
					sView -= game->m_stMCursor.sZ/120;				
			}
...
{
	if(game->m_stMCursor.sX < m_X || game->m_stMCursor.sX > m_X + sSizeX ||
		game->m_stMCursor.sY < m_Y || game->m_stMCursor.sY > m_Y + sSizeY)
	{
		return false;
	}
	else {
