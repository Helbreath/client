...
BOOL DXC_ddraw::bInit(HWND hWnd)
{
 ...
 int            iS, iD;
	...
	SetRect(&m_rcClipArea, 0,0, 800, 600); // 800x600 Resolution xRisenx
	...
	if( m_bFullMode == TRUE )
	{
		...
		ddVal = m_lpDD4->SetDisplayMode(800, 600, 16,0,0); // 800x600 Resolution xRisenx
		//ddVal = m_lpDD4->SetDisplayMode(800, 600, 32,0,0); // 800x600 Resolution xRisenx
		if (ddVal != DD_OK) return FALSE;
		...
		if (ddVal != DD_OK) return FALSE;
		//SetRect(&m_rcFlipping, 0, 0, 640, 480); // our fictitious sprite bitmap ii
		SetRect(&m_rcFlipping, 0, 0, 800, 600); // 800x600 Resolution xRisenx
	}
	else
	{
		...
		SetWindowPos( hWnd, HWND_TOP, cx-400, cy-300, 800, 600, SWP_SHOWWINDOW); // 800x600 Resolution xRisenx
		memset( (VOID *)&ddsd, 0, sizeof(ddsd) );
		ddsd.dwSize = sizeof( ddsd );
	...
	m_lpBackB4 = pCreateOffScreenSurface(800, 600);  // 800x600 Resolution xRisenx
	if (m_lpBackB4 == NULL) return FALSE;

	// Pre-draw background surface
	//m_lpPDBGS = pCreateOffScreenSurface(640+32, 480+32);
	m_lpPDBGS = pCreateOffScreenSurface(800+32, 600+32); // 800x600 Resolution xRisenx
...
//#else
		...
		//ddVal = m_lpBackB4flip->BltFast( 0, 0, m_lpBackB4, &m_rcFlipping, DDSCAPS_VIDEOMEMORY ); // | DDBLTFAST_WAIT);
		//ddVal = m_lpFrontB4->Flip(m_lpBackB4flip, DDFLIP_NOVSYNC);
...
void DXC_ddraw::ChangeDisplayMode(HWND hWnd)
{
	...
	if( m_bFullMode == TRUE )
	{
		...
		SetWindowPos( hWnd, NULL, cx-400, cy-300, 800, 600, SWP_SHOWWINDOW); // 800x600 Resolution xRisenx
		...
		//SetRect(&m_rcFlipping, cx-320, cy-240, cx+320, cy+240); // our fictitious sprite bitmap is 
		SetRect(&m_rcFlipping, cx-400, cy-300, cx+400, cy+300); // 800x600 Resolution xRisenx
		m_bFullMode = FALSE;
	}
	else
	{
...
void DXC_ddraw::ClearBackB4()
{
	...
	memset((char *)ddsd2.lpSurface, 0, ddsd2.lPitch * 600); // 800x600 Resolution xRisenx
	m_lpBackB4->Unlock(NULL);
}
...
void DXC_ddraw::DrawShadowBox(short sX, short sY, short dX, short dY, int iType)
{
	...
	if(dX >= 800) // 800x600 Resolution xRisenx
		dX = 800-1; // 800x600 Resolution xRisenx
	...
	if(sX >= 800) // 800x600 Resolution xRisenx
		return;

	if(dY >= 600) // 800x600 Resolution xRisenx
		dY = 600-1; // 800x600 Resolution xRisenx
	...
	if(sY >= 600) // 800x600 Resolution xRisenx
		return;
...
void DXC_ddraw::PutPixel(short sX, short sY, WORD wR, WORD wG, WORD wB)
{
	...
	if ((sX < 0) || (sY < 0) || (sX > 799) || (sY > 599)) return; // 800x600 Resolution xRisenx
