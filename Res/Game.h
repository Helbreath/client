...
#define DOUBLECLICKTIME			200//300
...
{
public:
	void IconPanelShortCut(int num, bool magic, bool use);
	void RemoveIconPanelShortCut(int num, bool magic, bool all);

	void DrawDialogBox_OnlineList();
	void DlgBoxClick_OnlineList();
	void NotifyMsg_OnlineOnGame(char * pData);

	uint32 CrusadeMaxGMG, AresdenGMGAmount, ElvineGMGAmount;
	void NotifyMsg_CrusadeStats(char * pData);
	void DrawCrusadeInfo();

	bool GetIPAddress();

	void NotifyMsg_GuildQuestReward(char *pData);
	void NotifyMsg_Guild_QuestContents(char *pData);
	...
	void ResponseGuildQuestList(char * pData);
   ...
   void DlgBoxClick_GuildContribute();//58
	...
	void DlgBoxDoubleClick_IconPanel();
	...
	Vector<string> * GetItemName(CItem * pItem);
	...
	void MakeSpecialSprite( char* FileName, short sStart, short sCount, bool bAlphaEffect = TRUE);
	void MakeTileSpr( char* FileName, short sStart, short sCount, bool bAlphaEffect = TRUE);
	void MakeSpecialTileSpr( char* FileName, short sStart, short sCount, bool bAlphaEffect = TRUE);
	void MakeEffectSpr( char* FileName, short sStart, short sCount, bool bAlphaEffect = TRUE);
	void MakeSpecialEffectSpr( char* FileName, short sStart, short sCount, bool bAlphaEffect = TRUE);
	...
	void AddEventList(char const * const pTxt, char cColor = 0, BOOL bDupAllow = TRUE, BOOL Top = TRUE);
	...
	void ButtonString(CDialogBox & dlg, int button, char const * const pString, bool enabled, Alignment pos = POS_CENTER);
	...
	struct {
		BOOL bIsGuildQuestCompleted;
		short sTargetType, sTargetCount, sCurrentCount;
		short sContribution, sMJReward, sGoldReward;
		char cTargetName[22];
	} m_GuildQuest;
	...
	struct {
		...
		int TargetType;
		int MaxCount;
		int Contribution, MJReward, GoldReward;
		char TargetName[22];
	} m_stGuildQuestList[50];

	int m_iGuildQuestCount;
...
//	int m_iAccountStatus;
	...
	short m_sMagicShortCutPanle[5];
	short m_sItemShortCutPanle[5];
	...
	short m_sShortCut[2]; // Snoopy: 6 shortcuts
